...
The Fairbrix Core daemon can be configured to act as a trusted "border
router", implementing the fairbrix wire protocol and relay, making
...
The ZeroMQ feature in Fairbrix Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building fairbrixd:
...
For instance:
    ...
    $ fairbrixd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/fairbrixd.tx.raw \
...
These options can also be provided in fairbrix.conf.
...
From the perspective of fairbrixd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into fairbrixd directly. Furthermore, no information is
...
using. Fairbrixd appends an up-counting sequence number to each
