...
3. All content of the data directory, except for `fbx.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `fbx.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `fairbrixd` or `fairbrix-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
